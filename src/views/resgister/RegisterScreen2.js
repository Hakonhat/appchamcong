...
//           </View>
//         </View>
//         <Text style={styles.selectedGender}>Giới tính đã chọn: {}</Text>
//         <View style={styles.inputContainer}>
...
//   try {
//     const response = await axios.post(`${URL_IMAGES}/upload`, formData, {
//       headers: {
...
const registerImage = async (images, personId) => {
  ...
  // Hàm để chuyển đổi tệp ảnh thành dữ liệu nhị phân và thêm vào FormData
  const addImageToFormData = async (imageURI, index) => {
    ...
    formData.append('images', blob, fileName); // Thêm dữ liệu nhị phân của ảnh vào FormData
  };

  // Lặp qua mỗi đường dẫn ảnh và thêm vào FormData
  for (let i = 0; i < images.length; i++) {
    await addImageToFormData(images[i], i);
  }

  // Thêm ID của người vào dữ liệu biểu mẫu
  formData.append('personId', personId);
